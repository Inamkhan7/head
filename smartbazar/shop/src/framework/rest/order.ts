...
export function useCreateQuoteOrder() {
  const router = useRouter();

  const { mutate: createQuoteOrder, isLoading } = useMutation(client.orders.create, {
    onSuccess: (data) => {
      if (data?.tracking_number) {
        router.push(`${ROUTES.QUOTES}/${data?.tracking_number}`);
      }
    },
    onError: (error) => {
      const {
