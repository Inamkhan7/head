...
function OrderView({ order }: any) {
  ...
  let billingAddressDetail:any = localStorage.getItem('billingAddressDetail');
  let shippingAddressDetail:any = localStorage.getItem('shippingAddressDetail');

  billingAddressDetail = JSON.parse(billingAddressDetail);
  shippingAddressDetail = JSON.parse(shippingAddressDetail);
  return (
      ...
      <div className="mx-auto w-full max-w-screen-lg rounded border bg-light p-6 shadow-sm sm:p-8 lg:p-12">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2 md:mb-12 lg:grid-cols-4">
          ...
          <div className="rounded border border-border-200 py-4 px-5 shadow-sm">
            ...
            <p className="text-sm text-body-dark">
              {order?.payment_gateway ?? "Cash on Delivery" ?? 'N/A'}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="w-full lg:w-1/2 ltr:lg:pl-3 rtl:lg:pr-3">
            ...
            <div>
              ...
              {!isEmpty(order?.shipping_address) || !isEmpty(shippingAddressDetail) && (
                <p className="mt-5 flex text-body-dark">
                  <strong className="w-4/12 text-sm font-semibold text-heading">
                  ...
                  :
                  <span className="w-8/12 text-sm ltr:pl-4 rtl:pr-4 ">
                    {formatAddress(order?.shipping_address!) ?? formatAddress(shippingAddressDetail)}
                  </span>
                </p>
              )}
              {!isEmpty(order?.billing_address) || !isEmpty(billingAddressDetail) && (
                <p className="mt-5 flex text-body-dark">
                  <strong className="w-4/12 text-sm font-semibold text-heading">
                  ...
                  :
                  <span className="w-8/12 text-sm ltr:pl-4 rtl:pr-4">
                    {formatAddress(order?.billing_address!) ?? formatAddress(billingAddressDetail)}
                  </span>
                </p>
              )}
            </div>
