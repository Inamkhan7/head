...
export declare type PriceSchedule = {
  OwnerID: string;
  ID: string;
  Name: string;
  ApplyTax: boolean;
  ...
  PriceBreaks: PriceBreak[]; // Array of PriceBreak objects
  Currency: string | null;
  SaleStart: string | null; // Use `Date | null` if working with Date objects
  SaleEnd: string | null;   // Use `Date | null` if working with Date objects
  IsOnSale: boolean;
  xp: any | null;
}
...
export interface UpdatePriceScheduleInput extends Partial<CreatePriceScheduleInput> {}
...
export declare type priceSchedulePaginator = {
  paginatorInfo: PaginatorInfo;
  data: Array<PriceSchedule>;
}
