...
const fetchQuoteOrders = async ({ queryKey }: QueryParamsType) => {
  const [_key, params] = queryKey;
  const url = `${API_ENDPOINTS.ORDERS}?xp.OrderType=Quote`;
  const {
    data: { data, ...rest },
  } = await Orders.all(url);
  return { orders: { data, paginatorInfo: mapPaginatorData({ ...rest }) } };
...
const useOrdersQuotesQuery = (params: QueryOptionsType = {}, options: any = {}) => {
  return useQuery<any, Error>([API_ENDPOINTS.ORDERS, params], fetchQuoteOrders, {
    ...options,
    keepPreviousData: true,
  });
};
export {useOrdersQuotesQuery , fetchQuoteOrders};
