...
import { useProductsQueryForOrder } from "@data/order/order-products.query";
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-2 xl:grid-cols-4 3xl:grid-cols-6 gap-4">
         
          {
            products?.data != undefined ?
              ...
              // products?.paginatorInfo?.map((product: Product) => (
                <ProductCard key={products?.paginatorInfo.id} item={products?.paginatorInfo} />
              // ))
          }
        </div>
      </div>
      {!products?.data?.length && !products?.paginatorInfo?.id ? (
